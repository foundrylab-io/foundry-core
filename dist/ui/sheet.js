...
function SheetOverlay({ className, ...props }) {
    return (_jsx(SheetPrimitive.Overlay, { className: cn("fixed inset-0 z-50 bg-black/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0", className), ...props }));
}
...
function SheetTitle({ className, ...props }) {
    return (_jsx(SheetPrimitive.Title, { className: cn("text-lg font-semibold text-foreground", className), ...props }));
}
function SheetDescription({ className, ...props }) {
    return (_jsx(SheetPrimitive.Description, { className: cn("text-sm text-muted-foreground", className), ...props }));
}
